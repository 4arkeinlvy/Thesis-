\chapter{TINJAUAN REFERENSI}
\label{ch:tinjauan}

Bab ini memaparkan landasan teoretis yang menjadi pijakan rancangan sistem klasifikasi multimodal pada penelitian ini. Pemaparan disusun dari konsep yang paling umum, yaitu \textit{smart city} dan partisipasi publik, menuju konsep yang lebih spesifik, yaitu akuisisi data melalui \textit{web scraping}, kerangka kecerdasan buatan, pra-proses citra dan teks, ekstraksi fitur dan reduksi dimensi, mekanisme \textit{attention}, arsitektur \textit{Transformer}, keluarga \textit{backbone} visual dan \textit{encoder} teks modern, fusi multimodal, ekosistem inferensi ONNX, permodelan UML, kontainerisasi Docker, hingga \textit{stack} aplikasi mobile dan \textit{web framework}. Setiap bagian menjelaskan definisi, motivasi, sejarah singkat, dan formulasi matematis bila diperlukan sebagai dasar bagi keputusan metodologis yang dijelaskan pada Bab Metode Penelitian.

Pemilihan rujukan memprioritaskan publikasi mutakhir periode 2021--2026 untuk menggambarkan kondisi terkini, khususnya pada \textit{backbone} visual dan \textit{encoder} teks yang dipakai. Namun, beberapa rujukan fondasional yang lebih lama dari lima tahun tetap disitasi pada sumber aslinya karena bersifat normatif atau merupakan asal-usul metode yang dipakai langsung pada penelitian ini, contohnya algoritma CatBoost \citep{prokhorenkovaCatBoost2018}, arsitektur \textit{Transformer}, notasi UML \citep{boochUML2005}, \textit{C4 model} \citep{brownC42018}, serta standar \textit{flowchart} ISO 5807 \citep{iso5807}. Rujukan semacam ini diperlakukan sebagai pengecualian fondasional, bukan sebagai klaim kebaruan.
...
\subsection{Alur dan Pustaka \textit{Web Scraping}}
...
\subsubsection{Citra Digital sebagai Tensor}
...
\subsubsection{\textit{Resize} dan Interpolasi}
...
\subsubsection{Aspek Rasio dan \textit{Square Padding}}
...
\subsubsection{Normalisasi Per-Kanal}
...
\subsubsection{\textit{Layout} Tensor dan Konversi Warna}
...
\subsubsection{Kompresi JPEG dan EXIF}
...
\subsubsection{Catatan tentang Augmentasi Data}
...
\subsection{Pemrosesan Data Teks}
\label{sec:textproc-detail}

\subsubsection{Pembersihan dan Normalisasi}
...
\begin{tabular}{p{2.4cm} p{3.7cm} p{3.6cm} p{2.6cm}}
\toprule
...
\subsubsection{\textit{Embedding}: \textit{Lookup Matrix} dan Dimensi}
...
\subsection{Penggunaan dalam \textit{Pipeline Embedding}}
...
\subsection{Keterbatasan dan Alternatif}
...
\textit{Multi-head attention} adalah perluasan \textit{single-head attention} yang menjalankan beberapa proyeksi linear paralel dari \textit{query}, \textit{key}, dan \textit{value}, kemudian mengkonkatenasi keluaran setiap \textit{head} dan memproyeksikannya kembali ke dimensi $d_{\text{model}}$. Motivasinya adalah bahwa satu \textit{head} terbatas pada satu pola hubungan antar token, sementara teks dan citra memuat banyak pola hubungan secara simultan. Setiap \textit{head} bekerja pada subruang berdimensi $d_k = d_{\text{model}}/h$ sehingga total komputasi tetap sebanding dengan \textit{single-head attention}.

Secara formal, untuk setiap \textit{head} $i \in \{1, \dots, h\}$ didefinisikan tiga matriks proyeksi yang dapat dilatih:
...
Dimensi $d_k$ pada setiap \textit{head} biasanya jauh lebih kecil daripada $d_{\text{model}}$, contohnya $d_{\text{model}} = 768$ dengan $h = 12$ menghasilkan $d_k = 64$. Pada model bahasa modern jumlah \textit{head} umumnya berkisar antara 8 dan 32, dan penambahan \textit{head} melampaui jumlah optimal sering kali tidak meningkatkan performa serta membuat sebagian \textit{head} menjadi redundan.
...
Selain DINOv3, penelitian ini juga mengevaluasi dua \textit{backbone} visual modern sebagai pembanding pada eksperimen ablasi, yaitu EVA-02 dan Hiera, yang diuraikan pada dua subbagian berikut.
...
Hasil eksperimen (Bab~\ref{ch:hasil}) menunjukkan DINOv3 dan EVA-02 unggul, sementara Hiera konsisten paling lemah pada domain laporan publik.
...
\subsection{\textit{Encoder} Teks Pembanding}
\label{sec:textencoder-pembanding}

Sebagai pembanding pada eksperimen ablasi, penelitian ini juga mengevaluasi empat \textit{encoder} teks lain, yaitu BERT/mBERT, IndoBERT, Cendol-mT5, dan BGE-M3, sebelum keluarga E5 dipilih pada \textit{pipeline} akhir.

\subsubsection{BERT dan mBERT}
...
\textbf{Diagram struktur} memodelkan elemen sistem yang relatif statis. Tiga jenis yang paling sering dipakai adalah Diagram Kelas (\textit{class diagram}: kelas, atribut, metode, dan relasi), Diagram Komponen (\textit{component diagram}: komponen yang dapat di-\textit{deploy} beserta antarmukanya), dan Diagram Deployment (\textit{deployment diagram}: pemetaan komponen ke simpul perangkat keras). \textbf{Diagram perilaku} memodelkan dinamika sistem; empat jenis yang relevan adalah Diagram \textit{Use Case} (interaksi aktor dengan fungsionalitas), Diagram \textit{Sequence} (urutan pesan antar objek pada sumbu waktu), Diagram Aktivitas (\textit{activity diagram}: alur kerja dengan simpul keputusan dan paralel), dan Diagram \textit{State Machine} (keadaan objek dan transisinya). Penelitian ini memakai \textit{use case}, \textit{activity}, \textit{class}, \textit{sequence}, serta \textit{component} dan \textit{deployment diagram} untuk mendokumentasikan rancangan SmartCityApps (Bab~\ref{ch:metode}).
